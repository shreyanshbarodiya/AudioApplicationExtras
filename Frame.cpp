	...
	if (infile)
	{
		...
		while(hop_time<50)
		{
			...
			for (int k=0; k<WINSIZE; k++){
				fprintf (outfile1,"%.4f	%d \n", float(k)/meta->sample_rate, buff16[k]);
			}	

			for (int k=0; k<WINLEN; k++){
				fprintf (outfile2,"%.4f	%.2f \n", float(10*k)/meta->sample_rate, databuff[k]);
			} 
			*/
			...
			/*
			for (int k=0; k<WINLEN; k++){
				fprintf (outfile2,"%.4f	%d \n", hop_time+k/double(samp_rate), databuff[k]);
			}*/	
			/*

			for (int k=0; k<WINLEN; k++){
				fprintf (outfile3,"%.4f	%.2f \n", hop_time+k/double(samp_rate), data[k]);
			}
			...
			*/			
			...
			for(int i=0; i<WINLEN; i++){
				...
				//cout<<"Norm calculation - I: "<<i<<" norm so far"<<norm<<endl;;
			}
				
			//Computing autocorrelation
			double acf[acf_len]; 
			double acfval;		
			//cout<<"Min lag: "<<lag_min<<endl;
			//cout<<"Max lag: "<<lag_max<<endl;
			//cin>>dummy;
			for(int i=lag_min; i<lag_max; i=i+acf_jump){
				...
				//cout<<"Lag: "<<i<<" ACF: "<<acfval<<" i: "<< int((i-lag_min)/acf_jump)<<endl;
				acf[int((i-lag_min)/acf_jump)] = double(acfval/norm);		//assign this correctly
				//cout<<"ACF: "<<acfval<< endl;
				//cin>>dummy;
			...
			for(int i=1; i<acf_len; i++){
				//cout<< "Comparison in progress"<<acf[i]<<" with "<<acf[maxpos]<<endl;
				if(acf[i]>acf[maxpos]) {
					//cout<<i<<" is greater than "<<maxpos<<endl;
					maxpos = i;
				}
			}
			...
			//cout<<"Max pos is: "<<maxpos<<" with ACF = "<<acf[maxpos]<<endl;
